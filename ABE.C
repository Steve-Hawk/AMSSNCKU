// $Id: ABE.C,v 1.1.1.1 2012/02/03 08:46:28 zjcao Exp $
#ifdef newc
#include <iostream>
#include <iomanip>
#include <fstream>
...
// read parameter from file
	...
	if(sgrp == "ABE")
	{
              if ( skey == "checkrun" )  checkrun = atoi(sval.c_str()); 
         else if ( skey == "checkfile")  strcpy(checkfilename, sval.c_str()); 
         else if ( skey == "Steps" )     Steps  = atoi(sval.c_str());
	 else if ( skey == "StartTime" ) StartTime = atof(sval.c_str());
	 else if ( skey == "TotalTime" ) TotalTime = atof(sval.c_str());
	 else if ( skey == "DumpTime")   DumpTime = atof(sval.c_str());
	 else if ( skey == "CheckTime") CheckTime = atof(sval.c_str());
	 else if ( skey == "AnalysisTime")  AnasTime = atof(sval.c_str());
	 else if ( skey == "Courant")    Courant = atof(sval.c_str());
	 else if ( skey == "Symmetry" )  Symmetry = atoi(sval.c_str());
	 else if ( skey == "small dissipation") numepss = atof(sval.c_str());
	 else if ( skey == "big dissipation") numepsb = atof(sval.c_str());
         else if ( skey == "Analysis Level" )  a_lev = atoi(sval.c_str());
         else if ( skey == "Max mode l" )     maxl  = atoi(sval.c_str());
         else if ( skey == "detector number" )     decn  = atoi(sval.c_str());
         else if ( skey == "farest detector position" )  maxrex  = atof(sval.c_str());
         else if ( skey == "detector distance" )     drex  = atof(sval.c_str());
	}
      }
      inf.close();
    }
...
//===========================the computation body====================================================
   ...
   ADM=new bssn_class(Courant,StartTime,TotalTime,DumpTime,CheckTime,AnasTime,
...
//=======================caculation done=============================================================
  ...
  if(myrank==0)cout<<"Simulation is successfully done!!"<<endl;
   MPI_Finalize(); 
