//$Id: Block.C,v 1.1.1.1 2012/02/03 08:46:28 zjcao Exp $
#include <iostream>
#include <iomanip>
#include <fstream>
#include <cstdlib>
...
Block::Block(int DIM, int *shapei, double *bboxi,int ranki, int ingfsi, int fngfsi, int levi):
rank(ranki),ingfs(ingfsi),fngfs(fngfsi),lev(levi)
{
...
#error Both Cell and Vertex are defined
    ...
    for(int j=0;j<shape[i];j++) X[i][j] = bbox[i] + j*h;
#else
#ifdef Cell
    double h = (bbox[dim+i]-bbox[i])/shape[i];
    for(int j=0;j<shape[i];j++) X[i][j] = bbox[i] + (j+0.5)*h;
#else
...
Block::~Block()
{
  ...
  if(myrank == rank)
  {
    ...
    for(int i=0; i<ingfs; i++) free(igfs[i]);
    delete[] igfs;
    for(int i=0; i<fngfs; i++) free(fgfs[i]);
    delete[] fgfs;
    X[0]=X[1]=X[2]=0;
    igfs=0;
    fgfs=0;
