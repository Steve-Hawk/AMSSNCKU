//$Id: Parallel.C,v 1.1.1.1 2012/02/03 08:46:28 zjcao Exp $
#include "Parallel.h"
#include "fmisc.h"
...
#define SEARCH_SIZE 5
      ...
      n=max(1,(block_size+split_size/2)/split_size);

      maxnx=max(1,shape[0]/min_width[0]); maxnx=min(cpusize,maxnx);
      maxny=max(1,shape[1]/min_width[1]); maxny=min(cpusize,maxny);
      maxnz=max(1,shape[2]/min_width[2]); maxnz=min(cpusize,maxnz);
      ...
      max_fxfz=max(fx,fz);
      max_fyfz=max(fy,fz);
      nx=mnx=max(1,min(maxnx,(int)(pow(n,1.0/3.0)*fx/max_fyfz)));
      ny=mny=max(1,min(maxny,(int)(pow(n,1.0/3.0)*fy/max_fxfz)));
      nz=mnz=max(1,min(maxnz,(int)(pow(n,1.0/3.0)*fz/max_fxfy)));
      dn=abs(n-nx*ny*nz);
      hmin_width=min(shape[2]/nz,shape[1]/ny);
      hmin_width=min(hmin_width,shape[0]/nx);
	 ...
	 for (cny=max(1,mny-SEARCH_SIZE); cny<=(min(mny+SEARCH_SIZE,maxny)); cny++)
	   for (cnx=max(1,mnx-SEARCH_SIZE); cnx<=(min(mnx+SEARCH_SIZE,maxnx)); cnx++)
     	   {
	    cmin_width=min(shape[2]/cnz,shape[1]/cny);
    	    cmin_width=min(cmin_width,shape[0]/cnx);
		                    ...
		                    bool periodic)
{
  ...
  while(PLi)
  {
    ...
    if(lev != PLi->data->lev) cout<<"Parallel::distribute CAUSTION: meet Patches for different level: "<<lev<<" and "<<PLi->data->lev<<endl;
    PLi = PLi->next;
  }  
...
#error Not define Vertex nor Cell
	    ...
	    ng=new Block(dim,shape_here,bbox_here,n_rank++,ingfsi,fngfsi,PP->lev);  //delete through KillBlocks
//	    ng->checkBlock();
...
// set PP->ble      
  ...
  if(reacpu < cpusize*2/3)
  {
     ...
     if(myrank == 0) cout<<"Parallel::distribute CAUSTION: level#"<<lev<<" uses essencially "<<reacpu<<" processors vs "<<
...
// we assume index has already memory space
   ...
   mu = new int[DIM];
   mu[0] = 1;
   for(int i=1;i<DIM;i++) mu[i] = mu[i-1]*shape[i-1];
...
#error Not define Vertex nor Cell
   ...
   for(int i=0;i<DIM;i++) {indi[i]=illi[i];indo[i]=illo[i];}
/*
//check start index   
   for(int i=0;i<DIM;i++)
   {      
...
*/
   ...
   for(int i = 0;i<ial;i++)
   {
	...
	for(int j=0;j<DIM;j++)
	{
     ...
     for(int j=1;j<DIM;j++)
     {
       if(indi[j-1] == iuui[j-1]+1) {indi[j-1]=illi[j-1];indi[j]++;}  // carry 1 to next digital
       else break;
     }
     indo[0]++;
     for(int j=1;j<DIM;j++)
     {
       if(indo[j-1] == iuuo[j-1]+1) {indo[j-1]=illo[j-1];indo[j]++;}
       else break;
     }
   }
/*
...
// round at 4 and 5
  ...
  while(DumpList)
  {
	...
	{
	  char filename[100];
   	  if(tag)sprintf(filename,"%s_Lev%02d-%02d_%02d_%s_%05d.bin",tag,BP->lev,Bi,myrank,VP->name,ncount);
   	  else   sprintf(filename,   "Lev%02d-%02d_%02d_%s_%05d.bin"    ,BP->lev,Bi,myrank,VP->name,ncount);
  ...
  int ncount=int(time/dT + 0.5);
   ...
   while(DumpList)
   {
	...
	Bp=Bp->next;
     ...
     {
       char filename[100];
       if(tag)sprintf(filename,"%s_Lev%02d-%02d_%s_%05d.bin",tag,PP->lev,grd,VP->name,ncount);
       else   sprintf(filename,   "Lev%02d-%02d_%s_%05d.bin"    ,PP->lev,grd,VP->name,ncount);
  ...
  int ncount=int(time/dT + 0.5);
   ...
   while(DumpList)
   {
	...
	Bp=Bp->next;
     ...
     {
       char filename[100];
       if(tag)sprintf(filename,"%s_Lev%02d_%s_%05d.bin",tag,PP->lev,VP->name,ncount);
       else   sprintf(filename,   "Lev%02d_%s_%05d.bin"    ,PP->lev,VP->name,ncount);
		               ...
		               double *poXb, int ordn, double *SoA, int Symmetry)
{
   ...
   for(int i=0;i<3;i++) poX[i] = poXb[i];

   switch(Symmetry)
   {  
      case 2:
          for(int i=0;i<3;i++) if(poX[i] < 0) {poX[i] = -poX[i]; asgn = asgn*SoA[i];}
	  break;
      case 1:
                               if(poX[2] < 0) {poX[2] = -poX[2]; asgn = asgn*SoA[2];}
   }

   int extb[3];

   for(int i=0;i<3;i++) extb[i] = ext[i];
...
#endif    
      case 2:
          if(poX[0] < (ghost_width-1)*(CoX[0][1]-CoX[0][0])) extb[0] = extb[0]+ghost_width-1;
          if(poX[1] < (ghost_width-1)*(CoX[1][1]-CoX[1][0])) extb[1] = extb[1]+ghost_width-1;
      case 1:
          if(poX[2] < (ghost_width-1)*(CoX[2][1]-CoX[2][0])) extb[2] = extb[2]+ghost_width-1;
#else
#ifdef Cell
      case 2:
          if(poX[0] < (ghost_width-0.5)*(CoX[0][1]-CoX[0][0])) extb[0] = extb[0]+ghost_width;
          if(poX[1] < (ghost_width-0.5)*(CoX[1][1]-CoX[1][0])) extb[1] = extb[1]+ghost_width;
      case 1:
          if(poX[2] < (ghost_width-0.5)*(CoX[2][1]-CoX[2][0])) extb[2] = extb[2]+ghost_width;
#else
#error Not define Vertex nor Cell
#endif  
#endif
...
#error Both Cell and Vertex are defined
#endif    
          for(int j=0;j<ghost_width-1;j++)       CoXb[i][j] = -CoX[i][ghost_width-1-j];
	  for(int j=ghost_width-1;j<extb[i];j++) CoXb[i][j] = CoX[i][j-ghost_width+1];
#else
#ifdef Cell
          for(int j=0;j<ghost_width;j++)       CoXb[i][j] = -CoX[i][ghost_width-1-j];
	  for(int j=ghost_width;j<extb[i];j++) CoXb[i][j] = CoX[i][j-ghost_width];
#else
#error Not define Vertex nor Cell
#endif  
#endif	
	...
	{
          for(int j=0;j<extb[i];j++) CoXb[i][j] = CoX[i][j];
	}
      }
...
#error Not define Vertex nor Cell
	 ...
	 datab[i] = datain[lon]*sgn;
      ...
      for(int i=0;i<3;i++) delete[] CoXb[i];
      delete[] datab;
   }
   else
   {
		               ...
		               double *poX, int ordn)
{
   ...
   for(int i=0;i<DIM;i++) 
   {
     ...
     if(ind[i] == -1 && feq(poX[i],CoX[i][0],(CoX[i][1]-CoX[i][0])/2)) ind[i] = 0;
/*
     if(ind[i] < 0)
     {
...
*/    
     if(ind[i] == ext[i]-ordn+1 && feq(poX[i],CoX[i][ext[i]-ordn/2],(CoX[i][1]-CoX[i][0])/2)) ind[i] = ext[i]-ordn-1;
/*     
...
*/     
   ...
   NN[DIM-1] = ordn;
   for(int i=DIM-2;i>=0;i--) NN[i] = NN[i+1]*ordn;
   ...
   { 	
	 ...
	 {
	    for(int k=0;k<ordn;k++) funcvals[k]=DDd[k+j*ordn];
	 ...
	 DDd = DDd1;
      ...
      {
	for(int j=0;j<ordn;j++) funcvals[j]=DDd[j];
...
MyList<Parallel::gridseg> *Parallel::clone_gsl(MyList<Parallel::gridseg> *p,bool first_only)
{
  ...
  while(p)
  {
    ...
    if(first_only) {np->next=0; return np;}
    pq=q;
    p=p->next;
  }
  return np;
...
MyList<Parallel::gridseg> *Parallel::gs_subtract(MyList<Parallel::gridseg> *A,MyList<Parallel::gridseg> *B)
{
  ...
  for(int i=0;i<dim;i++)
  {
    if(B->data->llb[i] > A->data->uub[i] || B->data->uub[i] < A->data->llb[i]) return clone_gsl(A,true);
  ...
  for(int i=0;i<dim;i++)
  {
    cut_plane[i]=max(A->data->llb[i],B->data->llb[i]);
...
#error Both Cell and Vertex are defined
#endif    
          C->data->uub[i]=max(C->data->llb[i],cut_plane[i]-DH[i]);
#else
#ifdef Cell
          C->data->uub[i]=max(C->data->llb[i],cut_plane[i]);
#else
#error Not define Vertex nor Cell
#endif  
#endif	 
...
#error Not define Vertex nor Cell
    ...
    cut_plane[i+dim]=min(A->data->uub[i],B->data->uub[i]);
...
#error Both Cell and Vertex are defined
#endif    
	  C->data->llb[i]=min(C->data->uub[i],cut_plane[i+dim]+DH[i]);
#else
#ifdef Cell
	  C->data->llb[i]=min(C->data->uub[i],cut_plane[i+dim]);
#else
#error Not define Vertex nor Cell
#endif  
#endif
...
// always make return and A, B distinct	
  ...
  while(B)
  {
     ...
     while(C1)
     {
	if(C0) C0->catList(gs_subtract(C1,B));
	else   C0 = gs_subtract(C1,B);
	C1=C1->next;
     }
     if(C) C->destroyList();
     else  return 0;
...
MyList<Parallel::gridseg> *Parallel::gs_and(MyList<Parallel::gridseg> *A,MyList<Parallel::gridseg> *B)
{
  ...
  for(int i=0;i<dim;i++)
  {
    llb[i] = max(A->data->llb[i],B->data->llb[i]);
    uub[i] = min(A->data->uub[i],B->data->uub[i]);
    if(llb[i] > uub[i])
    {
	flag = true;
	break;
...
MyList<Parallel::gridseg> *Parallel::build_ghost_gsl(Patch *Pat)
{
	  ...
	  gs->next=0;
          ...
          if(!cgsl) cgsl = gs_subtract(gs,gsb);
	  else      cgsl->catList(gs_subtract(gs,gsb));
		          ...
		          MyList<Parallel::gridseg> **out_src,MyList<Parallel::gridseg> **out_dst)
{
	  ...
	  Parallel::gridseg *sd=s->data;
		...
		llb[i] = max(sd->llb[i],dd->llb[i]);
		uub[i] = min(sd->uub[i],dd->uub[i]);
...
//		else if(int(2*(llb[i]-dd->llb[i])/DH+0.4)%2 == 1) lb = llb[i]+DH/2;
	        if(lb > ub+min(SH,DH)/2) {flag = false; break;}  // special for isolated point
#else
#ifdef Cell
// |------|
// |-------------|
...
//		if     (int(2*(llb[i]-sd->llb[i])/SH+0.4)%2 == 1) lb = llb[i]-SH/2;
//		else if(int(2*(llb[i]-dd->llb[i])/DH+0.4)%2 == 1) lb = llb[i]-DH/2;
	        if(ub-lb < min(SH,DH)/2) {flag = false; break;}  // even for isolated point, it has a cell belong to it
#else
#error Not define Vertex nor Cell
#endif  
#endif 	
...
MyList<Parallel::gridseg> *Parallel::divide_gsl(MyList<Parallel::gridseg> *p, Patch *Pat)
{
   ...
   while(p)
   {
      if(cgsl) cgsl->catList(divide_gs(p,Pat));
      else     cgsl = divide_gs(p,Pat);
      p = p->next;
   }
...
#error Not define Vertex nor Cell
   ...
   int NN=1;
   for(int i=0;i<dim;i++) NN = NN*num[i];
...
// we assume dst and src locate on the same Patch
                if(dd->llb[i] < Pat->bbox[i])          llb[i] = max(sd->llb[i],dd->llb[i]+Pat->bbox[dim+i]-Pat->bbox[i]);
		else if(dd->llb[i] > Pat->bbox[i+dim]) llb[i] = max(sd->llb[i],dd->llb[i]-Pat->bbox[dim+i]+Pat->bbox[i]);
		else                                   llb[i] = max(sd->llb[i],dd->llb[i]);

		if(dd->uub[i] < Pat->bbox[i])          uub[i] = min(sd->uub[i],dd->uub[i]+Pat->bbox[dim+i]-Pat->bbox[i]);
		else if(dd->uub[i] > Pat->bbox[dim+i]) uub[i] = min(sd->uub[i],dd->uub[i]-Pat->bbox[dim+i]+Pat->bbox[i]);
		else                                   uub[i] = min(sd->uub[i],dd->uub[i]);
#ifdef Vertex
#ifdef Cell
#error Both Cell and Vertex are defined
#endif   
	        if(llb[i] > uub[i]+SH/2) {flag = false; break;}  // special for isolated point
#else
#ifdef Cell
	        if(llb[i] > uub[i]) {flag = false; break;}
#else
#error Not define Vertex nor Cell
#endif  
#endif 	
	     ...
	     if(flag)
	     {
		   ...
		   double SH=sd->Bg->getdX(i),DH=dd->Bg->getdX(i);
                   ...
                   if(dd->llb[i] < Pat->bbox[i])          d2->data->llb[i] = llb[i]-Pat->bbox[dim+i]+Pat->bbox[i];
 		   else if(dd->llb[i] > Pat->bbox[i+dim]) d2->data->llb[i] = llb[i]+Pat->bbox[dim+i]-Pat->bbox[i];
		   else                                   d2->data->llb[i] = llb[i];

		   if(dd->uub[i] < Pat->bbox[i])          d2->data->uub[i] = uub[i]-Pat->bbox[dim+i]+Pat->bbox[i];
		   else if(dd->uub[i] > Pat->bbox[dim+i]) d2->data->uub[i] = uub[i]+Pat->bbox[dim+i]-Pat->bbox[i];
		   else                                   d2->data->uub[i] = uub[i];
#ifdef Vertex
#ifdef Cell
#error Both Cell and Vertex are defined
#endif    
...
void Parallel::checkgsl(MyList<Parallel::gridseg>* pp,bool first_only)
{
  ...
  if(myrank==0)
  {
   if(!pp) cout<<"Parallel::checkgsl meets empty gsl"<<endl;
   while(pp)
   {
    if(pp->data->Bg) cout<<"on node#"<<pp->data->Bg->rank<<endl;
    else             cout<<"virtual grid segment"<<endl;
    cout<<"shape: (";
    for(int i=0;i<dim;i++) 
    {
       if(i<dim-1) cout<<pp->data->shape[i]<<",";
       else        cout<<pp->data->shape[i]<<")"<<endl;
    }
    cout<<"range: (";
    for(int i=0;i<dim;i++) 
    {
       if(i<dim-1) cout<<pp->data->llb[i]<<":"<<pp->data->uub[i]<<",";
       else        cout<<pp->data->llb[i]<<":"<<pp->data->uub[i]<<")"<<endl;
    }
...
MyList<Parallel::gridseg> *Parallel::build_buffer_gsl(MyList<Patch> *PatL)
{
	  ...
	  if(cgsl)
	  {
	     ...
	     gs = gs->next;
	     if(gs) while(gs->next) gs = gs->next;
	  }
	  else
	  {
			  ...
			  int Symmetry)
{
  ...
  while(varl)
  {
    ...
    varl=varl->next;
       ...
       {
	  int Npts;
          if(myrank==BP->data->rank) Npts = BP->data->shape[0]*BP->data->shape[1]*BP->data->shape[2];
	  MPI_Bcast(&Npts,1,MPI_INT,BP->data->rank,MPI_COMM_WORLD);
	  double *pox[3];
	  for(int i=0;i<3;i++) pox[i] = new double[Npts];
	  ...
	  for(int i=0;i<3;i++) MPI_Bcast(pox[i],Npts,MPI_DOUBLE,BP->data->rank,MPI_COMM_WORLD);
	  double *res;
...
#error Not define Vertex nor Cell
    ...
    llb[i] = max(D->data->llb[i],B->data->llb[i]);
    uub[i] = min(D->data->uub[i],B->data->uub[i]);    
  ...
  for(int i=0;i<dim;i++)
  {
    vt=vt*(max(D->data->uub[i],B->data->uub[i]) - min(D->data->llb[i],B->data->llb[i]));
    vo=vo*(uub[i]-llb[i]);
  ...
  if((vd+vb-vo)/vt > ratio)
  {
     ...
     for(int i=0;i<dim;i++)
     {
       C->data->uub[i] = max(D->data->uub[i],B->data->uub[i]);
       C->data->llb[i] = min(D->data->llb[i],B->data->llb[i]);
#ifdef Vertex
#ifdef Cell
#error Both Cell and Vertex are defined
#endif    
...
#error Not define Vertex nor Cell
     ...
     if(D->data->Bg == B->data->Bg) C->data->Bg = D->data->Bg;
     else                           C->data->Bg = 0;
...
#error Both Cell and Vertex are defined
#endif    
  for(int i=0;i<dim;i++) DH[i] = (A->data->uub[i]-A->data->llb[i])/(A->data->shape[i]-1)/2;
#else
#ifdef Cell
  for(int i=0;i<dim;i++) DH[i] = (A->data->uub[i]-A->data->llb[i])/A->data->shape[i]/2;
#else
#error Not define Vertex nor Cell
#endif  
#endif
...
// direction i touch, other directions overlap		  
		...
		if((C1->data->llb[j]-C2->data->llb[j])*(C1->data->uub[j]-C2->data->llb[j])>0 && 
		   (C2->data->llb[j]-C1->data->llb[j])*(C2->data->uub[j]-C1->data->llb[j])>0) flag=false;
...
// for efficiency we ask the width of the patch at least 2(buffer+ghost+BD ghost)    
    if(uub[i]-llb[i] < DH[i]*2*(buffer_width+2*ghost_width)) return false;  //here this is valid for both vertex and cell
  }
...
void Parallel::fill_level_data(MyList<Patch> *PatLd,MyList<Patch> *PatLs,MyList<Patch> *PatcL,
			       ...
			       MyList<var> *tmList,int Symmetry,bool BB)
{
...
// time interpolation part
     if(BB)  prepare_inter_time_level(PatcL,FutureList,StateList,OldList,
		                      tmList, 0);  // use SynchList_pre as temporal storage space
     else    prepare_inter_time_level(PatcL,FutureList,StateList,
		                      tmList, 0);  // use SynchList_pre as temporal storage space
//restrict first~~~>
     {
...
{  
  ...
  double **pox;
  pox=new double*[dim];
  for(int j=0;j<dim;j++) pox[j]=new double[1];
  for(int i=0;i<NN;i++)
  {
     ...
     while(PL)
     {
	  ...
	  if(XX[j][i]<PL->data->bbox[j]+lld[j] || XX[j][i]>PL->data->bbox[j+dim]-uud[j]) {flag=false; break;}
	  pox[j][0]=XX[j][i];
       }
       if(flag) {PL->data->Interp_Points(VarList,1,pox,Shellf+i*num_var,Symmetry); break;}
       PL=PL->next;
     }
     if(!PL) {checkpatchlist(PatL,false); return false;}
  }
  for(int j=0;j<dim;j++) delete[] pox[j];
  delete[] pox;
...
bool Parallel::point_locat_gsl(double *pox,MyList<Parallel::gridseg> *gsl)
{
   ...
   while(gsl)
   {
     for(int i=0;i<dim;i++)
     {
       if(pox[i] > gsl->data->llb[i] && pox[i] < gsl->data->uub[i]) flag = true;
       else
       {
	  flag = false;
	  break;
